...
# Official Termux package names
PACKAGES="lua51 python git nano which"
...
for pkg_name in $PACKAGES; do
    if dpkg -s "$pkg_name" > /dev/null 2>&1; then
        ...
        if pkg install -y "$pkg_name"; then
            ...
            # Retry once after updating repos
            print_warning "Retrying after repository refresh..."
            pkg update -y

            if pkg install -y "$pkg_name"; then
                print_success "$pkg_name installed on retry"
            else
                print_error "Could not install $pkg_name"
                exit 1
            fi
        fi
    fi
